...
//调用SM2接口生成SM2证书
func CreateCertificateToMem(template, parent *sm2.Certificate, pub *sm2.PublicKey, key bccsp.Key) (cert []byte, err error) {
	if key == nil {
		return nil, fmt.Errorf("Sign should not be emtpy")
	}
	pk := key.(*sm2PrivateKey).privKey
	return sm2.CreateCertificateToMem(template, parent, pub, pk)
}
