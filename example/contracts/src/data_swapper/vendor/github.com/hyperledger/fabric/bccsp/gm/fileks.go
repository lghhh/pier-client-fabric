...
func (ks *fileBasedKeyStore) storePublicKey(alias string, publicKey interface{}) error {
	...
	case *sm2.PublicKey:
		rawKey, err = sm2.WritePublicKeytoMem(publicKey.(*sm2.PublicKey), ks.pwd)
	default:
...
func (ks *fileBasedKeyStore) loadPublicKey(alias string) (interface{}, error) {
	...
	if privateKey, err := utils.PEMtoPublicKey(raw, ks.pwd); err == nil {
		return privateKey, nil
	}
	if privateKey, err := sm2.ReadPublicKeyFromMem(raw, ks.pwd); err == nil {
		return privateKey, nil
	}
	return nil, fmt.Errorf("Failed parsing private key [%s].", alias)
}
...
func (ks *fileBasedKeyStore) loadKey(alias string) ([]byte, error) {
	...
	key, err := utils.PEMtoSM4(pem, ks.pwd)
	if err != nil {
